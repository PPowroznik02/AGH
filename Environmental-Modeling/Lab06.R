...
xs<-nx/2.0
zs<-25
#ile bêdzie kroków czasowych dla sejsmogramu
nt<-et/dt+1;
...
#macierze dla pól ciœnieñ w czasie t+1, t i t-1
p<-matrix(nrow=nz,ncol=nx,0)
pm<-matrix(nrow=nz,ncol=nx,0)
pp<-matrix(nrow=nz,ncol=nx,0)
#trzeba znaleŸæ vmax do warunku stabilnoœci 
vmax<-2000
...
while (1)
{
  ...
  #przejœcie o krok do przodu z macierzami
  pm<-p
  p<-pp
  #warunek do zapisania próbki sejsmogramu (taki sam bêdziecie musieli zrobiæ dla animacji!!!)
...
# 
# num <- 1
# plot_list <- vector("list", 40)
# for (i in seq(1,40,1)) {
#   plot_list[[num]] <- plot_ly(x=ss[,i], y=st, type='scatter', mode='lines')%>%
#     layout(autosize = 'F', xaxis = Noax, margin = m)
#   num<-num+1
# }
# 
# fig <- subplot(plot_list, shareY = TRUE)%>% 
#   layout(autosize = 'F', title = list(text = "Subplots with Annotations"),
#          xaxis = list(
#            zeroline = FALSE,
#            showline = FALSE,
#            showticklabels = FALSE,
...
# 
# 
# plot_list[1] <- plot_ly(x=ss[,1], y=st, type='scatter', mode='lines')%>%
#   layout(xaxis = Noax)
# plot_list[2] <- plot_ly(x=ss[,100], y=st, type='scatter', mode='lines')%>%
#   layout(xaxis = Noax)
# fig <- subplot(plot_list, shareY = TRUE)%>% 
